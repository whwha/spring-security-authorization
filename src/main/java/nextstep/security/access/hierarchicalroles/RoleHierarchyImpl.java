...
public class RoleHierarchyImpl implements RoleHierarchy {
    ...
    private Map<String, Set<String>> rolesReachableInOneOrMoreStepsMap;
    ...
    private static Map<String, Set<String>> buildRolesReachableInOneOrMoreStepsMap(Map<String, Set<String>> hierarchy) {
        Map<String, Set<String>> rolesReachableInOneOrMoreStepsMap = new HashMap<>();

        // iterate over all higher roles form rolesReachableInOneOrMoreStepsMap
        for (String roleName : hierarchy.keySet()) {
            Set<String> rolesToVisitSet = new HashSet<>(hierarchy.get(roleName));
            Set<String> visitedRoleSet = new HashSet<>();

            while (!rolesToVisitSet.isEmpty()) {
                // take a role from the rolesToVisit set
                String lowerRole = rolesToVisitSet.iterator().next();
                rolesToVisitSet.remove(lowerRole);
                if (!visitedRoleSet.add(lowerRole) || !hierarchy.containsKey(lowerRole)) {
                    continue;
            ...
            rolesReachableInOneOrMoreStepsMap.put(roleName, visitedRoleSet);
        }
    ...
    public static final class Builder {
        ...
        public final class ImpliedRoles {
            ...
            public Builder implies(String... impliedRoles) {
                Assert.notEmpty(impliedRoles, "Implied roles must not be null or empty");
                Assert.noNullElements(impliedRoles, "Implied role name(s) must not be empty");
